 ...
 *
 * Usage:
 *   ./simviz_stradibot            -> no sound
 *   ./simviz_stradibot --sound    -> with sound
 */
...
static constexpr double kConstantTestVolume         = 0.15;
...
// ---- Audio Helper Function Declarations ----
...
int main(int argc, char *argv[])
{
	...
	redis_client.setEigen(CONTROL_POSITION_KEY,       Vector3d::Zero());
	redis_client.setEigen(BOW_FORCE_SENSOR_WORLD_KEY, Vector3d::Zero());
	redis_client.setEigen(BOW_FORCE_SENSOR_LOCAL_KEY, Vector3d::Zero());
	...
	T_link_sensor.translation()   = Eigen::Vector3d(0.0, 0.05, 0.35);
	...
	// ADD SOUND
	//--------------------------------------------------------------------------
	if (sound_enabled) initializeAudio(graphics);

	chai3d::cShapeSphere *control_sphere = graphics->createGoalSphere(
		control_position, 0.01, chai3d::cColorf(1.0, 0.65, 0.0), true);
	...
	while (graphics->isWindowOpen() && fSimulationRunning)
	{
		...
		if      (graphics->isKeyPressed(49)) { key_pressed = 1; cout << "1 key pressed\n"; }
		else if (graphics->isKeyPressed(50)) { key_pressed = 2; cout << "2 key pressed\n"; }
		else if (graphics->isKeyPressed(51)) { key_pressed = 3; cout << "3 key pressed\n"; }
		else if (graphics->isKeyPressed(52)) { key_pressed = 4; cout << "4 key pressed\n"; }
		else if (graphics->isKeyPressed(57)) { key_pressed = 9; cout << "9 key pressed\n"; }
		else                                 { key_pressed = 0; }
		redis_client.setDouble(KEYBOARD_INPUT_KEY, key_pressed);
		...
		//--------------------------------------------------------------------------
		if (sound_enabled) updateAudioPlayback();
...
void simulation(std::shared_ptr<SaiSimulation::SaiSimulation> sim)
{
	...
	while (fSimulationRunning)
	{
		...
		//--------------------------------------------------------------------------
		if (sound_enabled) updateAudioPhysics(sim);
...
void initializeAudio(std::shared_ptr<SaiGraphics::SaiGraphics> graphics)
{
	...
	// graphics->attachAudioDeviceToCamera(camera_name, audioDevice); // not available in this version of SaiGraphics
	...
	for (int i = 0; i < static_cast<int>(kStringConfigs.size()); ++i)
	{
		...
		if (buffer_loaded)
		{
			...
			std::cout << "Audio loaded: " << kStringConfigs[i].link_name << "\n";
		}
		else
		{
			std::cout << "ERROR: Could not load " << file_paths[i] << "\n";
...
void updateAudioPlayback()
{
	...
	for (int i = 0; i < static_cast<int>(kStringConfigs.size()); ++i)
	{
		if (!violinToneSources[i]) continue;

		const double measured_frequency = std::clamp(
			audio_states[i].frequency_hz, kMinAudibleFrequencyHz, kMaxAudibleFrequencyHz);
		const double target_frequency = kStringConfigs[i].open_string_frequency_hz;
		const double frequency_error  = std::abs(measured_frequency - target_frequency);
		...
		double target_gain = audio_states[i].in_contact
							 ? kConstantTestVolume * (0.15 + 0.85 * frequency_match_strength)
							 : 0.0;
