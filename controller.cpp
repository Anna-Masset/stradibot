 ...
 * 		in another terminal, do redis-cli, then get "sai::control::bow::force_controlled" so see the force, set "sai::control::bow::force_controlled"
 * 		same with "sai::control::bow::speed_controlled"
 */
...
void sighandler(int)
{
	...
	{
		uint8_t z = 0;
		[[maybe_unused]] auto r = write(serial_fd, &z, 1);
	}
}
...
{
	if (serial_fd < 0 || solenoid == last_sent_solenoid)
		return;
...
static vector<NoteEvent> parseMidi(const string &filename)
{
	...
	if (!f)
	{
		cerr << "Cannot open MIDI file: " << filename << "\n";
		return {};
	}
	...
	char magic[4];
	f.read(magic, 4);
	if (string(magic, 4) != "MThd")
	{
		cerr << "Not a valid MIDI file\n";
		return {};
	}
	...
	{
		double speed_multiplier = 2.0; // multiplies the secs
		double secs = 0.0;
	...
	for (auto &rn : raw_notes)
	{
		...
		if (dur > 0.0)
			events.push_back({s, fret, start, dur});
	}
...
// 	ee_force_desired << redis_client.getEigen(BOW_FORCE_CONTROLLED_KEY);
				 ...
				 const Vector3d &initial_position)
{
	...
	if (key_once)
	{
		...
		{
			string_positions[0] = Vector3d(0.867011, -0.140365, 0.266309);
			string_orientations[0] << 0.0934987, -0.511516, -0.854171,
				-0.994479, -0.00692422, -0.10471,
				0.0476466, 0.859246, -0.50934;
			string_positions[1] = Vector3d(0.907412, -0.119718, 0.279737);
			string_orientations[1] << 0.0374749, -0.207924, -0.977427,
				-0.998624, 0.0281047, -0.0442662,
				0.0366743, 0.977741, -0.206585;
			string_positions[2] = Vector3d(0.924652, -0.117822, 0.274284);
			string_orientations[2] << -0.0108152, 0.0953699, -0.995383,
				-0.999024, 0.0416067, 0.0148412,
				0.0428301, 0.994572, 0.0948269;
			string_positions[3] = Vector3d(0.945377, -0.116711, 0.255744);
			string_orientations[3] << -0.063477, 0.480517, -0.874685,
				  ...
				  double transition_speed)
{
	...
	auto move_toward = [&](const Vector3d &waypoint)
	{
		...
		if (dist > step)
			ee_pos_desired += error.normalized() * step;
		else
			ee_pos_desired = waypoint;
	};
...
int main(int argc, char *argv[])
{
	...
	const double MAX_FORCE = 3.0;		// N

	// Manual bowing (original values)
	double manual_bow_speed = 0.06;
	...
	redis_client.setDouble(BOW_SPEED_CONTROLLED_KEY, manual_bow_speed);
	...
	redis_client.setDouble(BOW_FORCE_CONTROLLED_KEY, ee_force_desired(1));
	...
	while (runloop)
	{
		...
		case ControllerState::MANUAL_BOWING:
		{
			if (key_once && current_key >= 1 && current_key <= 4)
			{
				...
				if (requested != current_string)
				{
					...
					move_to_string_target = string_positions[target_string] + bow_displacement * new_bowing_dir;
			...
			manual_bow_speed = min(redis_client.getDouble(BOW_SPEED_CONTROLLED_KEY), MAX_BOW_SPEED);
			...
			ee_pos_desired = string_position + bow_displacement * bowing_dir;
			ee_force_desired(1) = -min(redis_client.getDouble(BOW_FORCE_CONTROLLED_KEY), MAX_FORCE);
			ee_ori_desired = string_orientation_world;
			break;
		}
		...
		case ControllerState::BOW_NOTE:
		{
			...
			if (!bow_note_initialized)
			{
				...
				midi_bow_speed_ = min(redis_client.getDouble(BOW_SPEED_CONTROLLED_KEY), MAX_BOW_SPEED);
